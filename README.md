...
- `manifest.json` — `author`, `version` and three modules, `css`, `favicon` and
  `patch`.
- `index.js` — builds the header, the pilot band, the links band and the footer as
  real DOM, and adds the TODAY badge and the current-date row highlight. It is
  DOM rather than CSS `content:` because `content:` text cannot be selected or
  clicked and the copy carries URLs people need. Filestash serves it from inside
  the zip at `/assets/<BUILD_REF>/plugin/branding.zip/index.js`, so
...
- `branding.css` — layout, colour and the rules hiding the write controls, which
...
drop the branding without an error. The `verify-branding` job in the build
workflow is the detector: it fetches the boot module from the running site and
fails if the injected import is absent.
