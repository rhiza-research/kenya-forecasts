...
//
// Adds four bands fixed around the file listing: the logo and title, the PILOT
// pill and disclaimer paragraph, two links, and a footer. Also marks the
// listing row whose date is the current date.
//
...
// The parenthesised repository is part of the sentence as written, and is built
// as a link rather than as text.
...
// The sentence names the website, so the name is the link.
...
// --- Bands ----------------------------------------------------------------
//
// The branding itself. Each function returns one fixed container that the
// stylesheet pins to an edge of the viewport.

// Builds the top container: the logo and title band, the PILOT pill and
// disclaimer band, and the links band. The three share one container so a
// single measured height covers all of them.
...
// date. Matched two ways: the archive names its folders with an ISO date, so
// the row path is the primary signal, and the rendered timestamp is a fallback.
